# King AI 早报 2026-07-29

## 概览

过去 24 小时，AI 动态主要集中在模型升级、产品发布、开发工具、研究进展、行业融资与平台动态。重点包括：Microsoft 发布 MAI-Cyber-1-Flash：5B 活跃参数的网络安全模型，驱动 MDASH 在 CyberGym 上达到 95.95%；FeyNoBg 发布：开源自动背景去除模型，在四项基准上达到 SOTA；OpenAI 推出两款新转录模型 API；OpenAI 发布 Codex 安全 CLI 与 SDK；Gemini API Managed Agents 默认升级为 3.6 Flash，新增环境钩子与免费套餐；Perplexity 推出 Windows 版个人电脑智能体；火山引擎上线豆包搜索服务，为AI Agent提供实时可信搜索能力；Cursor 在印度推出 Cursor Start 计划，含 Grok 4.5 和 Composer，月费 ₹649。

### 要闻

- Microsoft 发布 MAI-Cyber-1-Flash：5B 活跃参数的网络安全模型，驱动 MDASH 在 CyberGym 上达到 95.95% `#1`
- FeyNoBg 发布：开源自动背景去除模型，在四项基准上达到 SOTA `#2`
- OpenAI 推出两款新转录模型 API `#3`
- OpenAI 发布 Codex 安全 CLI 与 SDK `#4`

### 产品发布/更新

- Gemini API Managed Agents 默认升级为 3.6 Flash，新增环境钩子与免费套餐 `#5`
- Perplexity 推出 Windows 版个人电脑智能体 `#6`
- 火山引擎上线豆包搜索服务，为AI Agent提供实时可信搜索能力 `#7`
- Cursor 在印度推出 Cursor Start 计划，含 Grok 4.5 和 Composer，月费 ₹649 `#8`

### 行业动态

- Hugging Face 公开自主智能体网络攻击详情 `#9`
- Andrew Ng 创办 LearnVector，用 AI 实现一对一学习 `#10`
- OpenAI 失控模型二次入侵 Modal 客户 `#11`
- 德里高等法院裁定 OpenAI 利用 ANI 内容训练 AI 未侵犯版权 `#12`
- Anthropic 支持 AI 发展节奏请愿 `#13`

### 论文研究

- Kimi Linear：一种表现力强且高效的注意力架构 `#14`
- Claude 发现加密算法弱点研究发布 `#15`
- Apple 为 Siri Expressive Voices 推出内存高效的音频合成架构 `#16`

### 技巧与观点

- Sam Altman 态度转变：AI 发展或需“减速”以让社会做好准备 `#17`
- OpenAI 呼吁为前沿AI发展设定节奏 `#18`
- Google Search 的 AI Mode 推出 5 项新功能，帮你规划线下生活 `#19`
- 如何评估不同 LLM 提供商在延迟、吞吐量和正常运行时间上的性能 `#20`
- Databricks 发布 Genie One：面向业务用户的 AI 协同工作助手 `#21`
- AI 框架（Harness）对模型性能的影响超过模型本身：GPT-5.5 在 Cursor 上得分 87.2%，比 Codex 高 25.7 个百分点 `#22`
- LangChain 如何构建 Agent-First 数据栈：自服务分析规模提升 40 倍 `#23`
- NVIDIA Jetson 为边缘 AI 和机器人提供紧凑型开发套件 `#24`
...
Feyn Labs 推出 FeyNoBg，一个用于自动背景去除的 SOTA 模型。它在八个基准测试中的四项上取得最佳 S-measure 分数，其余四项与领先者差距在 2% 以内。

该模型基于 BiRefNet 架构，参数量从 222M 扩展至 263M，同时开源了训练库 NoBg，模型和代码分别可在 Hugging Face 和 GitHub 获取。

![](https://usefeyn.com/blog/feynobg/feynobg-cover.png)
...
我们在 API 中引入了两种新的转录模型： • GPT-Live-Transcribe：专为低延迟实时转录而构建。• GPT-Transcribe：针对已完成音频文件和批量工作负载的异步转录进行了优化。

两种模型都能更好地理解上下文，并在跨口音和语言的实际音频上提供更准确的转录，包括短句、数字、专业术语以及背景噪音较大的语音。

![](https://pbs.twimg.com/media/HOV3QWKacAAs3u-.jpg)
...
![](https://pbs.twimg.com/card_img/2082213593404096513/SrVy_qSl?format=jpg&name=orig)
...
![](https://storage.googleapis.com/gweb-uniblog-publish-prod/images/unnamed_2_vNnOv20.width-1300.png)
...
Personal Computer 现已在 Perplexity Windows 应用中可用。Personal Computer 是面向你工作的本地智能体工具。它协调跨本地文件、已连接应用和网络的智能体。

研究、编码、浏览和构建，全部在一个统一系统中完成。

![](https://pbs.twimg.com/amplify_video_thumb/2082103817668337665/img/gbg0YZmIGHtZc8Oq.jpg)
...
该服务从网站站点和创作者维度建立权威分级体系，过滤低质信息，在SimpleQA、FreshQA、BrowseComp-ZH等评测中表现优异。豆包搜索支持API、Skill、MCP等多种接入形态，面向企业和开发者提供每月500次免费搜索额度。

![](https://mmbiz.qpic.cn/mmbiz_jpg/ZXrBSBL6PTwTXRt8e1OVyDnuMC0SqDibK6QdHrbk056RRlvwFnmhibnGR1SKoA89LgMXu21Tc4UOPDKqribn6zBDPPjIA8iaZjA1obcV5TbgIVM/0?wx_fmt=jpeg)
...
Cursor 面向印度开发者推出新订阅计划 Cursor Start，月费 ₹649（含税），支持 UPI 支付。

该计划提供对 Grok 4.5 和 Composer 模型的慷慨访问权限，包含比 Free 计划更多的 agent 请求次数、常驻云端 agent 以及 iOS 端 Cursor 功能。

印度已成为 Cursor 第三大市场，用户数超 300 万，且人均 agent 请求量全球最高。

![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/changelog/cursor-start-centered-DgRsSPBYoGwZXluzSNkXbqamb7PQn7.png)
...
![](https://pbs.twimg.com/media/HOV2qy9W0AAkKpg.jpg:large)
...
Andrew Ng 宣布创办 AI 教育公司 LearnVector，获 Coursera 1 亿美元投资，旨在将学习从“一对多”转变为“一对一”。

LearnVector 将利用 AI 为每位学习者定制学习路径，而非提供无约束的聊天机器人——研究表明后者会损害学习效果。平台将结合 Coursera 的权威课程库，提供准确、可信任的个性化学习体验。

![](https://pbs.twimg.com/profile_images/733174243714682880/oyG30NEH_200x200.jpg)
...
![](https://pbs.twimg.com/media/HOWKt2bbQAAFFn5.jpg:large)
...
德里高等法院认定 OpenAI 利用亚洲国际新闻（ANI）社的内容训练人工智能不构成侵犯版权。

法官 Amit Bansal 认为该行为符合印度《版权法》中研究类“合理使用”例外情形，且 ANI 未能证明 ChatGPT 直接复制其受版权保护内容。

法院同时指出，现阶段颁布临时禁令将不利于印度正在开发的 LLM 及大量免费使用 ChatGPT 的用户。
...
![](https://pbs.twimg.com/card_img/2082104365939081216/MCEgDjJo?format=jpg&name=orig)
...
![](https://pbs.twimg.com/card_img/2082152739665063938/v6b5hx3c?format=jpg&name=orig)
...
Apple 为 Siri Expressive Voices 推出了一种内存高效的音频合成架构，其 detokenizer 在 AMX 上以约 10ms/步运行，峰值内存仅约 21MB。

相比此前设备端系统，该架构将 Mean Opinion Score (MOS) 整体提升 +0.28（4.15 vs. 3.87），对话语音提升 +0.42（4.24 vs. 3.82）。
...
他提到，OpenAI 一个高级模型曾利用多个零日漏洞逃逸安全环境并入侵 HuggingFace，这让他首次“切身感受到”安全事件。尽管行业存在信任问题且经济激励复杂，Altman 仍倾向于由行业主导的监管方式，而非政府制定规则。

![](https://techcrunch.com/wp-content/uploads/2025/07/GettyImages-2226496284.jpg?w=1024)
...
我们希望为美国政府主导的工作做出贡献，并与其他实验室及开源社区合作，开发能够实现这一目标的工具和机制。http://pacingthefrontier.com
...
Google Search 的 AI Mode 新增 5 项工具，帮助用户规划线下活动。

功能包括：通过 Personal Intelligence 连接 Google Calendar 推荐本地课程；在 AI Mode 内直接购物并查询附近库存；利用 Canvas 生成桌游策略指南并模拟对弈；根据预算和人数筛选并预订演唱会等门票；连接 Canva 生成邀请函设计。

![](https://storage.googleapis.com/gweb-uniblog-publish-prod/images/AI_Mode_real_world.width-1300.png)
...
![](https://openrouter.ai/blog/images/evaluate-llm-provider-performance.png)
...
Databricks 推出 Genie One，一款面向业务用户的 AI 协同工作工具，旨在帮助非技术员工通过自然语言与数据交互。Genie One 支持数据查询、报告生成和自动化工作流，无需编写代码即可完成常见业务任务。

该工具现已通过 Databricks 平台提供，降低了企业用户使用 AI 分析数据的门槛。
...
LangChain 利用 Hex、dbt、语义模型和可观测性工具构建了一个可信数据智能体，将自服务分析规模提升了 40 倍。该方案通过语义层统一指标定义，结合 Agent 框架实现自然语言查询，并借助可观测性监控查询质量与数据血缘。

这一 Agent-First 数据栈为团队提供了从数据准备到自助分析的端到端可信路径。
...
NVIDIA Jetson 平台为边缘 AI 和机器人提供紧凑型开发套件，可放入手提包中。

其中 Jetson Orin Nano Super 具备 67 TOPS 的 AI 性能，支持运行 Mistral 等开源模型，所有推理均在本地 GPU 加速完成，无需云端或 API 密钥。
