# King AI 早报 2026-07-28

## 概览

过去 24 小时，AI 动态主要集中在模型升级、产品发布、开发工具、研究进展、行业融资与平台动态。重点包括：Kimi K3 开源：2.8T MoE 模型与技术报告；Kimi K3 上线 Modal，支持无损加速推理；Kimi 发布视觉感知基准 PerceptionBench；SGLang 和 Miles 为月之暗面 2.8T 参数 Kimi K3 模型提供发布当日支持；Google AI Overviews 搜索结果出现率升至43%；NVIDIA 等多家行业领袖联合成立 Open Secure AI Alliance，推动 AI 安全与防御开源化；Cognizant 与 Anthropic 扩大合作，成为 Claude Partner Network 全球首要合作伙伴；Apple 提出 GH-ESD：面向实例级视觉任务的假设驱动错误切片发现方法。

### 要闻

- Kimi K3 开源：2.8T MoE 模型与技术报告 `#1`
- Kimi K3 上线 Modal，支持无损加速推理 `#2`
- Kimi 发布视觉感知基准 PerceptionBench `#3`
- SGLang 和 Miles 为月之暗面 2.8T 参数 Kimi K3 模型提供发布当日支持 `#4`

### 行业动态

- Google AI Overviews 搜索结果出现率升至43% `#5`
- NVIDIA 等多家行业领袖联合成立 Open Secure AI Alliance，推动 AI 安全与防御开源化 `#6`
- Cognizant 与 Anthropic 扩大合作，成为 Claude Partner Network 全球首要合作伙伴 `#7`

### 论文研究

- Apple 提出 GH-ESD：面向实例级视觉任务的假设驱动错误切片发现方法 `#8`

### 技巧与观点

- 用AI Skill自动生成可协作HTML PPT `#9`
- GitHub Copilot 发布“Harness”工作流：用单一工具完成原型、规划、实现与代码审查 `#10`
- 用Claude和Python构建技能驱动的金融分析智能体 `#11`
- OpenAI 称越来越多员工用 ChatGPT 做其他岗位的工作 `#12`
- GitHub Copilot app 入门指南：多 Agent 会话工作区与 Canvas 预览 `#13`
- 浪费20亿Token后，我开源了帮Agent定义目标的Leader.skill `#14`
- Anthropic 澄清立场：从未主张全面禁止开源权重模型，支持芯片出口管制与安全测试 `#15`
- OpenRouter 新增图像生成模型专用 API 端点 `#16`
...
![](https://pbs.twimg.com/media/HOPlk-WbQAAMkwd.jpg:large)
...
![](https://pbs.twimg.com/media/HOPslU9bAAAm2OK.jpg:large)
...
![](https://pbs.twimg.com/media/HOQVg7tasAAVpoo.jpg:large)
...
![](https://www.lmsys.org/blog/2026-07-27-kimi-k3-day0-support/opengraph-image?846984fa349aff93)
...
![](https://techcrunch.com/wp-content/uploads/2024/05/Sundar-AI-Overviews-IO-2024.png?resize=1200,698)
...
![](https://blogs.nvidia.com/wp-content/uploads/2026/07/osaia-logo-garden_press-kit_1920x1080_V4.png)
...
Cognizant 与 Anthropic 扩大合作，成为 Claude Partner Network 中的 Global Premier Partner，并将 Claude 嵌入其 Flowsource 等平台。

已有超过 3 万名员工完成 Claude 培训，其为一家生物制药公司构建的智能合约系统将合同审核时间缩短最高 40%，提取准确率超过 88%。
...
Vista 基于 bento PPT 改造了一个 Skill，输入内容或主题即可自动生成可编辑、在线演示并支持协作的 HTML PPT。

安装指令为 `npx skills add joeseesun/qiaomu-bento-ppt`，推荐使用 Kimi K3 或 Opus 4.8+ 等前端审美好的模型。

![](https://pbs.twimg.com/media/HOM3GIoaEAAkM-L.jpg:large)
...
OpenAI 分析超 80 万条与工作相关的 ChatGPT 消息后发现，43.5% 的岗位特定查询涉及另一职业，营销和工程任务交叉最多。用户用 AI 处理合同审查、数据分析、网站故障排查等原由专家负责的工作。

OpenAI 认为这是岗位职责正在变化的早期信号，该趋势在缺乏专业团队的小公司尤为明显。

![](https://the-decoder.com/wp-content/uploads/2026/07/openai_logo_vis-2.png)
...
GitHub Copilot app 将 AI 编码工具升级为多 Agent 会话工作区，支持同时管理多个任务线程而不丢失进度。

用户可为每个会话绑定项目上下文，通过 `/create-canvas` 命令在浏览器 Canvas 中预览 UI 并直接点选修改，还能启用 Agent Merge 自动处理 PR 审查反馈和合并冲突。

![](https://github.blog/wp-content/uploads/2026/07/GIT1050_THUMB_CFB_C.png?fit=1280%2C720)
...
作者开源了Leader.skill，用于将模糊的人类需求转化为Agent可独立执行数小时的目标任务书。

该Skill基于“目标七问”方法论，涵盖目的、完成态、反作弊、边界等维度，并推荐用Claude Fable 5或Kimi K3规划目标，再交由GPT-5.6 Sol或GLM-5.2等模型长程执行。项目已开源。

![](https://mmbiz.qpic.cn/mmbiz_jpg/2jjfQoZLoqXjZUeQCG058VPSChgeZBhtibTJia0ISfnqCp8LZ8PLVYPVKYxich0neylQeric70lD7IF8GApRvMw4hQKEdtIZFiaeAPoYgxtVBbicw/0?wx_fmt=jpeg)
...
![](https://openrouter.ai/blog/images/image-generation-models.png)
