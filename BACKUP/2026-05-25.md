...
1. **Luma Agents 实现规模化真实 UGC 广告生成** — X：Luma AI (@LumaLabsAI)
...
2. **OpenClaw 2026.5.22发布：性能优化与安全加固** — X：OpenClaw (@openclaw)
...
3. **TrapDoor供应链攻击：AI助手成新型攻击面** — X：Kim (@kimmonismus)
...
6. **格雷格·布罗克曼：那段差点让OpenAI覆灭的72小时** — Hacker News 热门（buzzing.cc 中文翻译）
   https://fs.blog/knowledge-project-podcast/greg-brockman

7. **Claude Code自动模式：多任务并行的关键技巧** — X：Boris Cherny (@bcherny)
...
8. **Pixverse角色设计工作流测试** — X：PixVerse (@PixVerse_)
